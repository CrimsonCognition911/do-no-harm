...
When both an OpenAI key and `DNH_EXAMINER_AGENT_ID` are configured, `POST /api/examiner` accepts the existing BFF's bounded delegation contract, reauthorizes its run-scoped examiner capability, invokes the saved Astra agent, records provisional examiner-only findings and returns frozen professor copy for supported or insufficient reasoning. A concern returns `pause_requested` and requests a server pause without racing speech against WebRTC shutdown; a provider failure requests an unscored technical pause. The synthetic GPT-Live-1 → BFF → saved Astra → GPT-Live-1 path and independent audio playback progress were observed on 13 September 2026; see [DNH-05 real-provider proof](DNH-05-LIVE-PROOF.md). There is still no durable provider/session storage, clinically enabled MI runtime, final scoring, production authentication, or real patient workflow. `GET /health` reports each configured synthetic integration; `GET /ready` remains 503. See the [Agents API boundary](contracts/agents-api.md) for the exact implemented slice and its limits.
...
- Next: persist delegation/provider recovery, add durable post-pause append/playback receipts, and complete production authentication and real-patient validation.
